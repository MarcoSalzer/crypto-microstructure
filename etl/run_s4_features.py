#!/usr/bin/env python3
# ==============================================================================
# Run Script — S4 Feature Engine
#
# Orchestrates the S4 feature pipeline for one or more asset-hours:
#   1. Scans s3_features/ to discover ready jobs
#   2. Validates that all required input files exist
#   3. Runs the S4 feature engine (S3 features -> S4 features)
#   4. Prints a structured execution summary
#
# The S4 output retains all previous-stage columns (S0–S3) plus new S4
...
#
# USAGE:
#   python -m etl.run_s4_features
#   python -m etl.run_s4_features --asset btc
#   python -m etl.run_s4_features --date 2026-02-16
#   python -m etl.run_s4_features --asset btc --date 2026-02-16 --hour 3
#   python -m etl.run_s4_features --dry-run
# ==============================================================================
...
from etl.engine.s4_feature_engine import (
...
_DEFAULT_INPUT_DIR = DATA_ROOT / "s3_features"
...
# Pattern: s3_features_btc_2026-02-16_03.parquet
_INPUT_PATTERN = re.compile(
    r"^s3_features_(\w+)_(\d{4}-\d{2}-\d{2})_(\d{2})\.parquet$"
)
...
def _discover_jobs(
    ...
    """Scan input directory for available S3 feature files."""
...
def _run_job(job: Job, verbose: bool = True) -> JobResult:
    t0 = time.time()
    try:
        df = build_s4_features_for_hour(
            s3_dir=job.input_dir,
            out_dir=job.out_dir,
            asset=job.asset,
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Run the S4 feature engine.\n\n"
            "Without arguments: auto-discovers all ready asset-date-hour\n"
            "combinations by scanning s3_features/."
        ),
    ...
    if not jobs:
        _warn("No S3 feature files found.")
