#!/usr/bin/env python3
# ==============================================================================
# Run Script — S3 Feature Engine
#
# Orchestrates the S3 feature pipeline for one or more asset-hours:
#   1. Scans s2_features/ to discover ready jobs
#   2. Validates that all required input files exist
#   3. Runs the S3 feature engine (S2 features -> S3 features)
#   4. Prints a structured execution summary
#
# The S3 output retains all previous-stage columns (S0–S2) plus new S3
...
#
# USAGE:
#   python -m etl.run_s3_features
#   python -m etl.run_s3_features --asset btc
#   python -m etl.run_s3_features --date 2026-02-16
#   python -m etl.run_s3_features --asset btc --date 2026-02-16 --hour 3
#   python -m etl.run_s3_features --dry-run
#   python -m etl.run_s3_features --no-context   # disable rolling-warmup context
# ==============================================================================
...
from etl.engine.s3_feature_engine import (
...
_DEFAULT_INPUT_DIR = DATA_ROOT / "s2_features"
...
# Pattern: s2_features_btc_2026-02-16_03.parquet
_INPUT_PATTERN = re.compile(
    r"^s2_features_(\w+)_(\d{4}-\d{2}-\d{2})_(\d{2})\.parquet$"
)
...
def _discover_jobs(
    ...
    """Scan input directory for available S2 feature files."""
...
def _run_job(job: Job, verbose: bool = True, use_context: bool = True) -> JobResult:
    t0 = time.time()
    try:
        df = build_s3_features_for_hour(
            s2_dir=job.input_dir,
            out_dir=job.out_dir,
            asset=job.asset,
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Run the S3 feature engine.\n\n"
            "Without arguments: auto-discovers all ready asset-date-hour\n"
            "combinations by scanning s2_features/."
        ),
        ...
        epilog=(
            ...
            "  %(prog)s --no-context                       # skip adjacent-hour warmup\n"
        ),
    )
    ...
    if not jobs:
        _warn("No S2 feature files found.")
    ...
    for i, job in enumerate(ready_jobs, 1):
        ...
        result = _run_job(job, verbose=not args.quiet, use_context=use_context)
