#!/usr/bin/env python3
# ==============================================================================
# Run Script — S5 Feature Engine
#
# Orchestrates the S5 feature pipeline for one or more asset-hours:
#   1. Scans s4_features/ to discover ready jobs
#   2. Validates that all required input files exist
#   3. Runs the S5 feature engine (S4 features -> S5 features)
#   4. Prints a structured execution summary
#
# The S5 output retains all previous-stage columns (S0–S4) plus new S5
...
#
# USAGE:
#   python -m etl.run_s5_features
#   python -m etl.run_s5_features --asset btc
#   python -m etl.run_s5_features --date 2026-02-16
#   python -m etl.run_s5_features --asset btc --date 2026-02-16 --hour 3
#   python -m etl.run_s5_features --dry-run
# ==============================================================================
...
from etl.engine.s5_feature_engine import (
...
_DEFAULT_INPUT_DIR = DATA_ROOT / "s4_features"
...
# Pattern: s4_features_btc_2026-02-16_03.parquet
_INPUT_PATTERN = re.compile(
    r"^s4_features_(\w+)_(\d{4}-\d{2}-\d{2})_(\d{2})\.parquet$"
)
...
def _discover_jobs(
    ...
    """Scan input directory for available S4 feature files."""
...
def _run_job(job: Job, verbose: bool = True) -> JobResult:
    t0 = time.time()
    try:
        df = build_s5_features_for_hour(
            s4_dir=job.input_dir,
            out_dir=job.out_dir,
            asset=job.asset,
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Run the S5 feature engine.\n\n"
            "Without arguments: auto-discovers all ready asset-date-hour\n"
            "combinations by scanning s4_features/."
        ),
    ...
    if not jobs:
        _warn("No S4 feature files found.")
